...
export default function PharmacySidebar() {
  ...
  return (
    ...
    >
      ...
      {/* --- LOGO SECTION --- */}
      <div className={`p-6 flex flex-col justify-center transition-all duration-300 ${isCollapsed ? 'items-center' : 'items-start'}`}>
        ...
        {/* Subtitle - Hides when collapsed */}
        <span className={`text-xs font-semibold text-content-secondary dark:text-dark-textSecondary mt-1 whitespace-nowrap overflow-hidden transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0 h-0' : 'w-auto opacity-100 h-auto ml-1'}`}>
            Pharmacy Portal
        </span>
      </div>
